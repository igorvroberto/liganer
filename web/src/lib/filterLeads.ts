...
const POT_ORDER: Record<string, number> = { A: 0, B: 1, C: 2, P: 3, X: 4 }
...
export function topAttackList(leads: Lead[], limit = 20): Lead[] {
  const scored = leads
    .filter((l) => l.classificacao_comercial === 'PROSPECT')
    .map((l) => {
      let score = 0
      if (l.potencial === 'A') score += 100
      else if (l.potencial === 'B') score += 50
      else if (l.potencial === 'C') score += 10
