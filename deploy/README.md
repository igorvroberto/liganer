...
Publicar em `https://vendas.liganer.com.br/orcamento/tubos-barras/`.
...
3. Em cada push na `main`, o Action faz `npm run build` e envia `dist/` para `/orcamento/tubos-barras/`.

### Caminho FTP deste projeto

```
ftp://acesso@liganer.com.br@ftp.liganer.com.br/vendas.liganer.com.br/orcamento/tubos-barras
```
...
| `FTP_SERVER_DIR` | `/vendas.liganer.com.br/orcamento/tubos-barras/` |
...
- Publica apenas o `index.html` da raiz, sem mexer em `/orcamento/blanks-slitters/`, `/orcamento/chapas-bobinas/`, `/orcamento/tubos-barras/` e `/prospeccao/`
...
1. Confirme que a pasta `orcamento/tubos-barras` existe no servidor (File Manager).
2. Crie **uma vez** no host a pasta `orcamento/tubos-barras/data/` (gravável pelo PHP). O Action **não** cria nem sobrescreve `data/`.
...
```json
{
  "saveUrl": "/orcamento/tubos-barras/api/budgets.php",
...
A pasta `dist/` sai com `base: /orcamento/tubos-barras/`.

## Checklist

- [ ] Secrets FTP preenchidos no GitHub (`FTP_SERVER_DIR` = `/vendas.liganer.com.br/orcamento/tubos-barras/`)
- [ ] Pasta `orcamento/tubos-barras/` existe no host
- [ ] Workflow verde em Actions após push na `main`
- [ ] `https://vendas.liganer.com.br/orcamento/tubos-barras/` abre com título `Liganer · Orçamento`
